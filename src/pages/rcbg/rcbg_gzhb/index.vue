			...
			<div class="datatable">
				<div class="datas">
					<el-tabs v-model="activeName" @tab-click="handleClick">
					    <el-tab-pane label="下级汇报" name="1" v-if="$store.state.userinfo.flag!=3">
							<div class="selectbar">
								<div>
									<el-input
									 	style="width: 200px !important;" 
										placeholder="搜索汇报" v-model="getdata.title"
								...
								<template>
									<el-table
										...
										<el-table-column 
											...
											width="150">
											<template slot-scope="scope">
												{{scope.row.pub_user==null?'暂无数据':scope.row.pub_user}}
											</template>
										</el-table-column>
										<el-table-column 
											prop=""
					    ...
					    <el-tab-pane label="我的工作汇报" name="2"  v-if="$store.state.userinfo.flag!=1">
							<div class="selectbar">
								<div>
									<el-input
										style="width: 200px !important;" 
										placeholder="搜索汇报" v-model="getdata1.title"
										@input="getdata1.page = 1;totalPage = 0;dataList1();">
									</el-input>
									<el-select
										style="margin-left: 10px;width: 120px !important;" 
										v-model="getdata1.status" placeholder="请选择状态"
										clearable
										@change="getdata1.page = 1;totalPage1 = 0;dataList1();">
										<el-option
											label="已查看"
											value="1">
										</el-option>
								...
								<div>
									<el-button type="primary" @click="$router.push('/add_hb')">工作汇报</el-button>
								</div>
							</div>
							<div class="selectdata">
								<template>
...
<script>
	...
	export default{
		...
		methods:{
			...
			dataList(){
				var that = this;
				rcbg.suboNotice(that.getdata).then(res => {
		...
		mounted() {
			...
			if(that.$store.state.userinfo.flag != 3){
				that.dataList();
				that.activeName = '1'
			}else{
				that.dataList1();
