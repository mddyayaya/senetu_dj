						...
						<template>
							<el-table
								...
								<el-table-column
									...
									fixed="right">
									<template slot-scope="scope">
										<el-button v-if="scope.row.is_up==0" @click="editData(scope.row)" type="text" size="small">编辑</el-button>
										<el-button v-if="" @click="del(scope.row)" type="text" size="small">删除</el-button>
									</template>
								</el-table-column>
							</el-table>
						</template>
			...
			getSummaries(param) {
				...
				columns.forEach((column, index) => {
					switch(column.property) {
						case "company":
							sums[0] = that.sum.company
							break;
						case "last_balance":
							sums[1] = parseFloat(that.sum.last_balance).toFixed(2)
							break;
						case "collection":
							sums[2] = parseFloat(that.sum.collection).toFixed(2)
							break;
						case "pay":
							sums[3] = parseFloat(that.sum.pay).toFixed(2)
							break;
						case "balance":
							sums[4] = parseFloat(that.sum.balance).toFixed(2)
							break;
						default:
							sums[index] = ""
							break;
